...
\item \textbf{Reply trees are not visibility graphs, and the difference is the
  ...
  effect on peer correction runs $+0.038$, $+0.080$ and $+0.095$ depending only
  on which is chosen (\S\ref{sec:rq1}).

\item \textbf{Total payoff is not a welfare measure under costly sanctioning.}
  In \textsc{NudgeSim} the intervention cuts propagation and quadruples peer
  correction, and $\sum\pi$ ranks it $125$ points below doing nothing.
...
\item \textbf{Mixed societies, not monocultures.} The obvious design runs one
  model per episode, which cannot answer the question the environment is built
  for: in a society of identical agents there is no \emph{other} agent to
  free-ride on. \textsc{NudgeSim} seats the whole roster in one society,
  rotating which model holds which seat so a model effect cannot be a network
  position wearing a different hat. Within a single episode---same claim, same
  topology, same intervention, same seed---policies differ by a factor of $5.6$
  in how often they pay the correction cost, which is H5 established under
  control a between-episode comparison cannot achieve (\S\ref{sec:rq6}).
...
control, a true-claim placebo arm, two ablations, an $N = 50$ scale check, a
perturbation arm and a mixed-society arm, for $1{,}550$ episodes. Claims are stratified by
...
\item[RQ5] Does an established correction norm survive a defector?
  (\S\ref{sec:rq5})
\item[RQ6] In a society of \emph{different} models, does one free-ride on
  another's correction? (\S\ref{sec:rq6})
\end{description}
...
\caption{Per-round outcomes by condition, pooled over backbones. The
intervener's entry produces an immediate step in endogenous peer correction
(right) and a slower divergence in propagation (left); the control arm, in
grey, is the baseline the four treatment arms are read against.}
...
\caption{The same corpus and seeds under three translations of reply structure
into visibility. The measured intervention effect (left) and the share of
neighbourhoods in which no citizen can observe any other (right) move together:
the translation that strands 88.6\% of neighbourhoods is also the one that
...
quadruples peer correction, and total payoff ranks it $125$ points below doing
nothing.}
\label{tab:welfare}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/welfare}
\caption{Where the payoff goes, by condition. Reputation damage is deadweight
in the aggregate and generating it is the intervener's function, so the measure
that sums it is determined before any agent acts.}
\label{fig:welfare}
\end{figure}
...
%----------------------------------------------------------------------
\subsection{RQ6: What one model does among others}
\label{sec:rq6}

Every study in this line that varies the backbone runs \emph{monocultures}:
one model per simulation, varied between simulations. That design answers
whether model choice moves the aggregate. It cannot answer whether one model
free-rides on another's contribution, because in a society of identical agents
there is no other model to free-ride on. Two of the four reference studies
therefore add a heterogeneous experiment---\citet{backmann2025moralsim} pair
each model against every other in an opponent matrix, and
\citet{tewolde2026coopeval} run replicator dynamics over an initially uniform
mixture---and both treat it as a headline research question rather than an
appendix.

The mechanisms \textsc{NudgeSim} measures make this unavoidable rather than
merely desirable. Conformity to a local majority, and declining to pay a cost
somebody else is already paying, are both claims about what one agent does in
response to \emph{another}. Under a monoculture those two agents are near
copies, so the design cannot separate ``this model corrects'' from ``this model
corrects when surrounded by itself''.

The mixed arm seats the whole roster in one society. Which model holds which
seat rotates by replication, so every model occupies every seat equally often:
seats differ in network position, and a fixed assignment would confound model
with position permanently---whatever sat in the best-connected seat would read
as a property of that model. Two contrasts follow, and neither is available
from the core grid.

\begin{table}[t]
\centering
\input{tables/composition}
\caption{Top: what each model does inside the \emph{same} episode, with claim,
topology and intervention held exactly fixed. Bottom: the same model's
correction rate among other models against its rate among copies of itself.
$^{*}$ significant after Holm correction. \textbf{Properties of the declared
data-generating process, not results about language models.}}
\label{tab:composition}
\end{table}

\paragraph{Backbone effects survive perfect control.} The top panel of
Table~\ref{tab:composition} is H5 measured \emph{within} an episode rather than
between episodes. Nothing differs between these rows except which policy sits in
the seat---not the claim, not the topology, not the intervention, not the
seed---and the correction rate still spans a factor of $5.6$. A between-episode
comparison cannot rule out that part of such a gap is claim or topology draw;
this one can, and the gap is larger under the tighter control rather than
smaller.

\paragraph{Whether effort depends on who the neighbours are: a null, so far.}
The bottom panel asks the second-order free-rider question between policies
rather than between an intervener and a society. On the surrogate it returns
nothing: no policy's correction rate detectably depends on whether its
neighbours are copies of itself, every interval straddles zero, and nothing
survives Holm correction. We report it because the contrast is the point of the
arm and because a null here is informative about the surrogate---its policies
respond to the \emph{count} of visible correction, not to who is doing it, so
there is no channel by which the identity of a neighbour could matter. That is a
property of an analytic policy, and precisely the assumption a language model
need not satisfy.

\pending{The same two panels on a mixed society of real models. This is the
result \S\ref{sec:rq3} and this section exist to produce, and the one that
distinguishes a benchmark of models from a study of a society made of them. The
arm costs nothing extra to run---a mixed episode issues the same number of calls
as a monoculture one, only spread across backbones.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/power}
\caption{What 30 replications per cell can detect. Only the intervention
contrasts sit inside the preregistered allocation; the tone contrasts are two
orders of magnitude outside it.}
\label{fig:power}
\end{figure}

Table~\ref{tab:validation} also carries a caution. The declared tone effect
needs roughly two thousand replications per cell for 80\% power, so its point
estimate at 30 or even 200 carries almost no information about its size: what
the table establishes is that the pipeline separates a present effect from an
absent one, not what the present effect is. A further source of movement is the
claim pool---a cell of a different size draws a different set of \textsc{Liar}
claims rather than more replications of the same ones---so the tone contrast is
under-powered in the claim dimension as well as the seed dimension.
...
\paragraph{What the line benchmarks.}
Every study in this line evaluates across vendors, and the rosters overlap
(Table~\ref{tab:rosters}). \textsc{Gpt-4o} appears in all four and is the
de-facto comparability anchor; open-weights models appear in all four; three of
the four include an Anthropic model. A single-vendor roster would make
\textsc{NudgeSim}'s model-dependence results incomparable with the papers it is
positioned against, so ours spans four vendors.
...
Study & $n$ & Models \\
\midrule
\textsc{GovSim} \citep{piatti2024govsim} & 15 &
  GPT-3.5, GPT-4, GPT-4-turbo, GPT-4o; Claude-3 Haiku/Sonnet/Opus;
  Llama-2 7B/13B/70B, Llama-3 8B/70B; Mistral-7B, Mixtral-8x7B;
  Qwen-72B/110B \\
\textsc{SanctSim} \citep{piedrahita2025sanctsim} & 9 &
  DeepSeek-V3, GPT-4o, GPT-4o-mini, Llama-3.3-70B;
  o1-mini, o1-preview, o3-mini (low/med/high) \\
\textsc{MoralSim} \citep{backmann2025moralsim} & 9 &
  DeepSeek-R1, o3-mini, Qwen-3-235B-A22B; Claude-3.7-Sonnet, DeepSeek-V3,
  Gemini-2.5-Flash, GPT-4o-mini, GPT-4o, Llama-3.3-70B \\
\textsc{CoopEval} \citep{tewolde2026coopeval} & 6 &
  Claude Sonnet 4.5, GPT-5.2, Gemini 3 Flash (reasoning \emph{and} base),
  GPT-4o, Qwen3-30B-A3B \\
\midrule
\textbf{\textsc{NudgeSim}} & \textbf{6} &
  \textbf{GPT-4o}; a current OpenAI reasoning model; \textbf{Claude Opus 5}
  with extended thinking \emph{and} without; \textbf{Llama-3.3-70B};
  \textbf{Qwen3-30B-A3B} \\
\bottomrule
\end{tabular}
\caption{Model rosters across the line. GPT-4o is the only model common to all
four prior studies. Boldface marks models \textsc{NudgeSim} shares with at least
one of them.}
\label{tab:rosters}
\end{table}

\paragraph{Model roster.}
The backbone factor has six levels chosen to serve three purposes at once.
\emph{Comparability}: GPT-4o is in all four prior studies, and Llama-3.3-70B and
Qwen3-30B each appear in two, so a model-dependence result here can be read
against theirs. \emph{Capability spread}: the roster runs from a 30B
open-weights model to a frontier one, which is what makes H5---whether backbone
choice moves the outcome more than intervention design does---measurable rather
than rhetorical. \emph{Reasoning}: two paired contrasts, one across models
within OpenAI and one across modes within fixed Anthropic weights.

The same-weights pairing follows \citet{tewolde2026coopeval}, who run Gemini 3
Flash with and without reasoning; \citet{backmann2025moralsim} note that
Claude-3.7-Sonnet and Gemini-2.5-Flash offer both modes but take the
non-reasoning variant for cost. It matters because
\citet{piedrahita2025sanctsim} report their largest split between reasoning and
traditional models while drawing that contrast \emph{across model families},
which confounds reasoning mode with everything else that differs between them.
Running it on a frontier model rather than a small one is the extension here,
and \S\ref{sec:cost} is why nobody had.

\paragraph{Open weights are not a budget compromise.} Llama-3.3-70B and
Qwen3-30B are served locally through the same OpenAI-compatible interface as the
hosted models, so they cost GPU time rather than tokens. They also answer a
question a hosted-only roster cannot: whether a result is bought with spend.
\citet{piedrahita2025sanctsim} find GPT-4o-mini out-contributing GPT-4o, so this
is not a formality.

\paragraph{Cost.} Each backbone level is charged its own replications of every
cell, so the grid grows linearly in models and the marginal cost of one more is
fixed and vendor-independent: \textbf{13.1M input and 1.43M output tokens}, or
43.4M output tokens if that level runs with reasoning enabled at 2{,}000 tokens
per call. Six levels give 2{,}080 episodes. We report the token budget rather
than a single dollar figure because six models span four rate cards that move
independently; Table~\ref{tab:cost} prices the Anthropic levels, where reasoning
tokens bill as output and the thinking arm dominates. Scheduling that arm for
the main grid and one high-power replication rather than for every calibration,
validation and re-run takes its programme cost from \$16{,}038 to \$1{,}866,
which is worth more than every caching lever combined.

Adding a vendor is a configuration change rather than an engineering one: the
environment reaches every hosted model in Table~\ref{tab:rosters} through one
OpenAI-compatible interface, and the game, metrics and analysis layers are
indifferent to which model---or whether any model---supplies the citizen
policies.
...
\textbf{One mixed composition, not a matrix.} The mixed arm seats the whole
roster at once. \citet{backmann2025moralsim} instead vary the opponent one at a
time, which separates ``model $A$ among strangers'' into ``$A$ facing $B$'',
``$A$ facing $C$'' and so on. That matrix is the stronger design and is a
straightforward extension of this one---each cell is a different seat
assignment---but it multiplies the arm by the roster size.
