...
module.exports.blocks = [
  ...
  evidence("The tone manipulation check, measured", [
    "The two arms are matched to within one token across the whole claim pool, with the payload surviving verbatim in both (Jaccard overlap 1.0). The check runs before any grid execution and blocks the run if it fails.",
  ]),
  ...
  table([20, 46, 34], [
    ...
    ["Stated credence (exploratory)", "0–100 self-reported credence at rounds 0, 6, 12 via out-of-band probes that never enter the shared feed.", "Interview-style probes; reported as model self-report only."],
  ]),
  ...
  p([t("Why welfare is not a simple payoff sum. ", { bold: true }),
     t("Because δ charges a citizen whenever a claim it is backing gets challenged, and generating exactly those challenges is the intervener's function, Σπ is mechanically anti-correlated with the treatment: the better the intervention works, the worse it scores, because reputation damage dominates the debits in any treated arm. This is not a ledger bug (the accounting matches §3.2 exactly and is checked against hand-computed episodes); it is why the standard treatment in experimental public-goods work reports welfare with and without punishment costs (Fehr & Gächter, 2002), and why conformity is reported separately from veracity rather than summed with it — conformity alone is maximised by the arm where nobody disagrees, which is not a measure of how well-informed the society is. The veracity component is the part of the payoff whose sign is stable across the two topology models in §5.2 (intervention beats control on accuracy under each, −27.4 against −30.0), which is why it is the primary welfare measure.")]),
  p([t("Why durability is area-under-the-curve rather than a half-life. ", { bold: true }),
     t("The Disseminator reposts every round and is by construction incorrigible, so FPR has a floor a short episode with a single intervener cannot push through, and a fixed absolute threshold is reached too rarely for a survival analysis to rest on. Area under the post-intervention FPR curve uses every round of every episode with no censoring, and answers the same question — how much pollution the intervention actually prevented. The relative-threshold survival analysis is kept as a secondary measure. The horizon is T = 12 rounds; extending it is the obvious lever if the secondary measure is to carry weight, but it does not rescue a fixed absolute threshold against a disseminator that never stops, which is why the primary measure is the one that uses every round rather than waiting for an event.")]),
