...
const front = [
  ...
  table([26, 74], [
    ...
    [{ text: "Date / version", bold: true }, { text: "September 2026 · v2.6 — the backbone factor is corrected from a monoculture to a mixed-society design after the reference implementation showed the original could not identify what RQ4/H5 ask (living document — comments welcome)", bold: true, bg: H.NEW_BG }],
  ]),

  h("0. What changed in v2.6, and why", 1),
  p([
  ...
  table([13, 30, 42, 15], [
    ...
    [{ text: "5.3, 5.11 (new in v2.6)", bg: H.NEW_BG },
     { text: "F3 seats one model per episode: all five citizens in an episode share a backbone, and the backbone changes between episodes. RQ4/H5 and the second-order free-rider question in §3.2 are both claims about how one agent responds to — or free-rides on — another agent's behaviour, and a monoculture has no other identity present to respond to. Two of the four reference studies already treat the heterogeneous case as a headline result rather than an appendix: MoralSim's RQ2 pairs every model against every other, and CoopEval's RQ3 runs replicator dynamics from a uniform mixture.", bg: H.NEW_BG },
     { text: "A mixed arm seats the full roster in one society per episode. Which model holds which seat rotates by replication, so every model occupies every seat equally often — enforced by a test — which keeps a seat's network position (some seats are better-connected than others) from being read as a property of whichever model happens to sit in it that replication.", bg: H.NEW_BG },
     { text: "On the surrogate, against real LIAR and PHEME-9 (proxy policies standing in for models, one per seat, same claim/topology/intervention/seed): peer-correction rate spans 5.4× across seated policies within a single episode (EPC 0.023 to 0.121) — a within-episode measurement of H5 that a between-episode comparison cannot rule out is partly claim or topology draw. Whether a policy's own rate depends on its neighbours' identity is null on the surrogate — largest gap −0.034 [−0.061, −0.006], not significant after Holm correction — which is informative about the surrogate: it responds to the count of visible correction, not to who is doing it, and that is precisely the assumption a language model need not share. Zero added cost: a mixed episode issues the same number of calls as a monoculture one, only spread across backbones.", bg: H.NEW_BG }],
  ]),
